...
- Every file in `engine/replay`, `engine/verification`, `engine/safety`, `engine/recording`, `engine/healing`, `engine/patching`, `engine/reporting`, and `engine/benchmark` must keep ≥ 90% line coverage from unit tests alone; `tests/unit/test_coverage_ratchet.py` enforces it.
...
| `make schema` | Regenerate `schemas/workflow.schema.json` and `schemas/bench-results.schema.json` from the domain models (a test fails when either is stale) |
...
| `make bench` | Run the published benchmark (levels 2, 3, 5 × seeds 1000–1019, both examples, every system, single-mutation cases) and write `benchmarks/results/chaos-results.json` and `scorecard.html`; a real-app results file, when present, follows the grid in its own section; exits 1 if Mendwork acts on a wrong element. `BENCH_MODEL=1` adds the configured model's column (ADR 0014) |
| `mendwork bench real-app` | Replay the Gitea pair's workflow on release B through every system and write `benchmarks/results/real-app-results.json`; needs a container runtime and the approved labels (ADR 0014) |
