...
async def sweep(browser: Browser, portal_url: str) -> AsyncIterator[PortalDriver]:
...
async def test_mutation_keeps_the_target_visible_and_working(
    sweep: PortalDriver, pair: HealPair
) -> None:
    if pair.page != "login":
        await ensure_signed_in(sweep)
    state = await sweep.open(pair.page, seed=pair.seed, only=[pair.mutation])
    ...
    await reveal_target(sweep, pair.target)
    element = await sweep.locate(pair.target)
    ...
    await assert_target_works(sweep, pair.target, how)
