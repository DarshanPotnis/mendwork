"""Every page script, and the run report's stylesheet, ship in the wheel and load from it, not only
from the source tree.

The wheel is built with uv, then a separate interpreter imports the adapter packages from
the wheel alone: ``-S`` skips site-packages (and the editable install's path to ``src``),
and PYTHONPATH names only the wheel, so importlib.resources can only read the files
...
ADAPTERS: Final = REPO / "src" / "mendwork" / "adapters"
SCRIPTS: Final = ADAPTERS / "browser_playwright" / "js"
STYLESHEET: Final = ADAPTERS / "report_html" / "report.css"
PROBE: Final = """
...
print(json.dumps({
    ...
    "stylesheet": (importlib.resources.files(report) / "report.css").read_text(encoding="utf-8"),
}))
"""


def test_every_page_script_and_the_report_stylesheet_load_from_the_built_wheel(
    tmp_path: Path,
) -> None:
    uv = shutil.which("uv")
    ...
    assert str(wheel) in reply["report_module"]
