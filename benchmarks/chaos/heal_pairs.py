"""The pair tables: a seed for every (mutation, eligible target) pair, by category.
...
HEAL: Final[Category] = "heal_expected"
ABSTAIN: Final[Category] = "abstain_expected"

TABLE_PATH: Final = Path(__file__).with_name("heal_pairs.json")
ABSTAIN_TABLE_PATH: Final = Path(__file__).with_name("abstain_pairs.json")
TABLE_PATHS: Final[Mapping[Category, Path]] = MappingProxyType(
    {HEAL: TABLE_PATH, ABSTAIN: ABSTAIN_TABLE_PATH}
)
_DESCRIPTIONS: Final[Mapping[Category, str]] = MappingProxyType(
    {
        category: (
            f"For every {category} mutation and every target it can take, a seed that makes "
            "?seed=<seed>&only=<mutation> apply exactly that pair."
        )
...
_ENGINE = """
  ...
  const ofCategory = (category) =>
    MUTATIONS.filter((m) => m.category === category && m.scope === "target");
...
_ELIGIBLE_PAIRS = f"""async (category) => {{
  {_ENGINE}
  return ofCategory(category).flatMap(
    (mutation) => eligible(mutation).map((key) => [mutation.id, key]),
  );
}}"""

_FIND_SEEDS = f"""async ([maxSeed, category]) => {{
  {_ENGINE}
  const found = [];
  for (const mutation of ofCategory(category)) {{
...
class HealPair(BaseModel):
    """One mutation on one target, and a seed that selects exactly that pair."""
...
def render_table(pairs: Iterable[HealPair], category: Category = HEAL) -> str:
...
async def eligible_pairs(
    page: Page, base_url: str, category: Category = HEAL
) -> frozenset[tuple[PageId, str, str]]:
    ...
    for page_id in PAGE_PATHS:
        ...
        for mutation, target in await page.evaluate(_ELIGIBLE_PAIRS, category):
...
async def find_seeds(
    page: Page, base_url: str, category: Category = HEAL, max_seed: int = MAX_SEED
) -> tuple[HealPair, ...]:
    ...
    for page_id in PAGE_PATHS:
        await open_unmutated(page, base_url, page_id)
        for mutation, target, seed in await page.evaluate(_FIND_SEEDS, [max_seed, category]):
            if seed is None:
...
async def generate() -> dict[Category, tuple[HealPair, ...]]:
    """Serve the portal, sign in, and find a seed for every eligible pair of each category."""
    with PortalServer(PORTAL_ROOT, host="127.0.0.1", port=0) as server:
        async with async_playwright() as playwright:
            ...
            try:
                ...
                return {
                    category: await find_seeds(page, server.url, category)
                    for category in get_args(Category)
                }
            finally:
                await browser.close()
...
def main() -> int:
    tables = asyncio.run(generate())
    for category, pairs in tables.items():
        path = TABLE_PATHS[category]
        path.write_text(render_table(pairs, category), encoding="utf-8")
        sys.stdout.write(f"Wrote {len(pairs)} {category} pairs to {path}\n")
    return 0
